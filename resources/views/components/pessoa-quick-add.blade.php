@props([
    'target',              // name do <select> de pessoas que recebe a nova opção
    'label' => 'Nova pessoa',
    'tipo' => 'fisica',    // fisica | juridica
])
{{-- Cadastro rápido de pessoa (padrão EDUQ): "+" abre modal, salva via AJAX e injeta a opção sem sair da tela --}}
<div x-data="pessoaQuickAdd('{{ $target }}', '{{ route('pessoas.quick') }}', '{{ csrf_token() }}', '{{ $tipo }}')" class="inline-block">
    <button type="button" @click="abrir()" class="text-xs text-cyan-600 hover:underline" title="Cadastrar pessoa sem sair da tela">
    ...
    <div x-show="aberto" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" @click.self="aberto = false">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            <h3 class="text-base font-semibold text-gray-800 mb-1">Cadastro rápido de pessoa</h3>
            <p class="text-xs text-gray-400 mb-4">Cria a pessoa na hora e já a seleciona no campo — depois você completa a ficha em Pessoa.</p>
            <div class="space-y-3">
                <input type="text" x-model="form.nome" placeholder="Nome / Razão social *" class="w-full border rounded-lg px-3 py-2 text-sm" @keydown.enter.prevent="salvar()">
                <div class="grid grid-cols-2 gap-3">
                    <input type="text" x-model="form.cpf" placeholder="CPF / CNPJ" class="w-full border rounded-lg px-3 py-2 text-sm">
...
@pushOnce('scripts')
<script>
function pessoaQuickAdd(targetName, url, token, tipo) {
    return {
        ...
        async salvar() {
            ...
            try {
                const r = await fetch(url, {
                    ...
                    body: JSON.stringify({ ...this.form, tipo }),
                });
