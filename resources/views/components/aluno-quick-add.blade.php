@props([
    'target',              // name do <select> de alunos que recebe a nova opção
    'label' => 'Novo',     // texto do gatilho
])
{{-- Cadastro rápido de aluno (padrão EDUQ): "+" abre modal, salva via AJAX e injeta a opção sem sair da tela --}}
<div x-data="alunoQuickAdd('{{ $target }}', '{{ route('alunos.quick') }}', '{{ csrf_token() }}')" class="inline-block">
    <button type="button" @click="abrir()" class="text-xs text-cyan-600 hover:underline" title="Cadastrar aluno sem sair da tela">
    ...
    <div x-show="aberto" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" @click.self="aberto = false">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            <h3 class="text-base font-semibold text-gray-800 mb-1">Cadastro rápido de aluno</h3>
            <p class="text-xs text-gray-400 mb-4">Cria o aluno na hora e já o seleciona no campo — depois você completa a ficha em Aluno (23).</p>
            <div class="space-y-3">
                <input type="text" x-model="form.nome" placeholder="Nome completo *" class="w-full border rounded-lg px-3 py-2 text-sm" @keydown.enter.prevent="salvar()">
...
@pushOnce('scripts')
<script>
function alunoQuickAdd(targetName, url, token) {
    return {
        ...
        async salvar() {
            if (!this.form.nome.trim()) { this.erro = 'Informe o nome do aluno.'; return; }
