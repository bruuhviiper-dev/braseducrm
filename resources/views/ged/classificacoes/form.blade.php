    ...
    <div class="bg-white">
        ...
        <form method="POST" action="{{ isset($classificacao) ? route('ged.classificacoes.update', $classificacao) : route('ged.classificacoes.store') }}" class="p-6 space-y-4">
            ...
            <div class="flex gap-3 pt-2">
                <button type="submit" class="px-8 py-3 bg-cyan-500 hover:bg-cyan-400 text-white rounded-full text-sm font-bold shadow-lg shadow-cyan-500/30">{{ isset($classificacao) ? 'Salvar Alteracoes' : 'Cadastrar' }}</button>
